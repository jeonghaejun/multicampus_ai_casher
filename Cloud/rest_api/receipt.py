...
def lambda_handler(event, context):
    ...
    phone_number = "+8210" + phone_arr[1] + phone_arr[2] 
    msg = "계산 내역입니다.\n"
    print(phone_number)
    
    
    for item in event["item"]:
        sql = "SELECT Product_name, Price FROM Item_info WHERE Item_id={}"
        sql = sql.format(item["id"])

        cursor.execute(sql)
        result = cursor.fetchall()
        print(result)
        total = (item["Qty"])*(result[0][1])
        
        
        msg += str(result[0][0]) + " " + str(item["Qty"]) + "개, " + str(total) + "원 구매하셨습니다.\n"
        
    
    try:
        response = client.publish(
            PhoneNumber = phone_number,
            Message=f'[스마트캐셔 모바일 영수증]\n{msg}',
        )
